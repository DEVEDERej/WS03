...
<section class="container mx-auto p-4 mt-4">
  <div class="rounded-lg shadow-md bg-white p-3">
    <div class="flex justify-between items-center">
      <a class="block p-4 text-blue-700" href="<?= url('/listings') ?>">
        <i class="fa fa-arrow-alt-circle-left"></i>
        Back To Listings
      </a>
      <div class="flex space-x-4 ml-4">
        <?php if (Authorization::isOwner($listing->user_id)) : ?>
        <a href="<?= url('/listings/edit/' . $listing->id) ?>" class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded">Edit</a>
        <!-- Delete Form -->
    ...
    <div class="p-4">
      ...
      <ul class="my-4 bg-gray-100 p-4">
        <li class="mb-2"><strong>Job Type:</strong> <?= htmlspecialchars($listing->company) ?></li>
        ...
        <li class="mb-2">
          ...
          foreach ($tags as $index => $tag) : ?>
            <span><?= htmlspecialchars(trim($tag)) ?></span><?= $index < count($tags) - 1 ? ', ' : '' ?>
          <?php endforeach; ?>
        </li>
        <?php endif; ?>
      </ul>
...
<section class="container mx-auto p-4">
  ...
  <div class="rounded-lg shadow-md bg-white p-4">
    ...
    <p>
      <?= nl2br(htmlspecialchars($listing->requirements ?? 'No specific requirements listed.')) ?>
    </p>
    <h3 class="text-lg font-semibold mt-4 mb-2 text-blue-500">Benefits</h3>
    <p>
      <?= nl2br(htmlspecialchars($listing->benefits ?? 'No benefits information available.')) ?>
    </p>
  </div>
  <p class="my-5">
